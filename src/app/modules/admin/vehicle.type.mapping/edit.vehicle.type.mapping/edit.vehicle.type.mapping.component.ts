...
export class EditVehicleTypeMappingComponent extends EditComponent implements OnInit {
    ...
    private async loadItem() {
        this.item = new VehicleTypeMappingEntity();
        if (this.id) {
            await this.service.item('vehicletypemapping', this.id).then((result: ResultApi) => {
                if (ResultApi.IsSuccess(result)) {
                    this.item = EntityHelper.createEntity(VehicleTypeMappingEntity, result.Object as VehicleTypeMappingEntity);
                } else {
    ...
    public async confirm(complete: () => void): Promise<boolean> {
        if (this.item) {
            ...
            if (await validation(this.item, columns)) {
                ...
                // upload Icon
                let images = await this.uploadIcon.upload();
                obj.Icon = images && images.length > 0 && images[0].Base64Data;

                // save
                return await this.service.save('vehicletypemapping', obj).then((result: ResultApi) => {
